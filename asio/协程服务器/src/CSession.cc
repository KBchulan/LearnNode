...
CSession::CSession(boost::asio::io_context &ioc, CServer *server) : _ioc(ioc), _close(false),
                                                                    _server(server), _socket(_ioc), _is_sending(false)
{
    ...
    { 
        try
        {
            while(!_close)
            {
                ...
                if(n == 0)
                {
                    std::cout << R"(Recvive peer closed!)" << '\n';
                    Close();
                ...
                std::cout << R"(Msg id is: )" << msg_id << '\n';
                if(msg_id > MAX_LENGTH)
                {
                    std::cerr << "Invalid msg id is: " << msg_id << '\n';
                    Close();
                    _server->ClearSession(_uuid);
                    co_return;
                }
                ...
                {
                    std::cerr << "Invalid msg len is: " << msg_len << '\n';
                    Close();
                    _server->ClearSession(_uuid);
                    co_return;
                }
                ...
                std::cout << R"(Receive data is: )" << _recv_msg_node->_data << '\n';
...
void CSession::Send(const char *msg, short max_length, short msg_id)
{
    ...
    {
        std::cout << R"(Session :)" << _uuid << " Send queue fulled, size is: " << MAX_SENDQUE << '\n';
        return;
    }
        ...
        boost::asio::co_spawn(_ioc, [self = shared_from_this()]() -> boost::asio::awaitable<void>
                              {
                                  ...
                                  {
                                      std::cerr << "Send coroutine error: " << e.what() << '\n';
                                      self->Close();
                                  }
                              },
                              boost::asio::detached);
...
void CSession::HandleWrite(const boost::system::error_code& error, std::shared_ptr<CSession> self)
{
    try
    {
        ...
        {
            std::cerr << R"(HandleWrite failed, error is :)" << error.message() << '\n';
            Close();
            _server->ClearSession(_uuid);
        }
    }
    ...
    {
        std::cerr << R"(HandleWrite exception is :)" << e.what() << '\n';
        Close();
        _server->ClearSession(_uuid);
    }
}
...
boost::asio::awaitable<void> CSession::HandleWrite()
{
    while (!_close)
    {
        ...
        try
        {
            ...
            std::cout << "Send message successfully, id: " << current_msg->GetMsgId()
                      << ", len: " << current_msg->_total_len << '\n';
        }
        catch (const std::exception &e)
        {
